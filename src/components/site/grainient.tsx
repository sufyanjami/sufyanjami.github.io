...
/**
 * Grainient: vendored from React Bits (reactbits.dev/backgrounds/grainient),
 ...
 *   2. `paused` prop. Upstream runs its rAF loop unconditionally. It already
 *      stops when offscreen or the tab is hidden, but has no notion of
 *      prefers-reduced-motion, which for a full-bleed animated backdrop is the
 *      case that matters most. Pausing renders a single static frame rather
 *      than a slow one.
 */
...
type GrainientCtx = {
  ...
  /** Loop controls, so the `paused` effect can drive the rAF without rebuilding the context. */
  start: () => void;
  stop: () => void;
  renderOnce: () => void;
};
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    ctxMap.set(container, { renderer, program, mesh, start: tryStart, stop: tryStop, renderOnce });
  ...
  // Pausing must not tear down the WebGL context, so it drives the existing loop.
  useEffect(() => {
